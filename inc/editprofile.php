...
{	// Aqui dentro realizaremos la actualización de los datos y validaciones.
	...
	if (count($errores) == 0)
	{
		...
		if($_POST['nickname'] != $fila['nickname'])
			{
			$sql = sprintf("update users set name='%s',surname='%s',password='%s',birthday='%s' where nickname='%s'", mysql_real_escape_string($_POST['name']), mysql_real_escape_string($_POST['surname']), encriptar($_POST['password']), cambiaf_mysql($_POST['birthday']), $_SESSION['nickname']);
			}
		else
			{
			$sql = sprintf("update users set name='%s',surname='%s',birthday='%s' where nickname='%s'", mysql_real_escape_string($_POST['name']), mysql_real_escape_string($_POST['surname']), cambiaf_mysql($_POST['birthday']), $_SESSION['nickname']);
			}
	...
	<form class="formulario" action="" method="post" autocomplete="off">
		<ul>
			...
			<li>
				...
				<input type="text" name="name" id="name" placeholder="Your name" size="10" maxlength="20" value="<?php if (empty($_POST['name'])) echo $fila['name'];  ?>"/>
			</li>
			<li>
				<label for="surname">Surname:</label>
				<input type="text" name="surname" id="surname" placeholder="Your surname here" size="20" maxlength="100" value="<?php if (empty($_POST['surname'])) echo $fila['surname']; ?>"/>
			</li>
			<li>
				<label for="password">Password:</label>
				<input type="password" name="password" id="password" size="10" maxlength="15" value="<?php 
				if (empty($_POST['password']))
					{
					echo $fila['password'];
					}
				?>"/>
				<input type="hidden" name="oldpassword" id="oldpassword" size="10" maxlength="15" value="<?php if (empty($_POST['password'])) echo $fila['password']; ?>"/>
			</li>
			<li>
				<label for="email">E-mail address:</label>
				<input type="email" name="email" id="email" placeholder="test@info.local" size="20" maxlength="50" disabled value="<?php if (empty($_POST['email'])) echo $fila['email']; ?>"/>
			</li>
			<li>
				<label for="birthday">Birthday:</label>
				<input type="date" name="birthday" id="birthday" value="<?php if (empty($_POST['birthday'])) echo $fila['birthday']; ?>"/>
			</li>
			<li>
